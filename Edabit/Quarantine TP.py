...
def tp_checker(home):
    People_need=(home["people"])*57
    total_tp=(home["tp"])*500
    avg=int(total_tp/People_need)
    if avg<=14:
        return "Your TP will only last {} days, buy more!".format(avg)
    else:
        return "Your TP will last {} days, no need to panic!".format(avg)
